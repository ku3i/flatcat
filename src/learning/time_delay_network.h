...
public:
    ...
    , output(target_size)
    , delta (output.size())
    , weights_hi(hidden.size(), td_input.size())
    , weights_oh(output.size(), hidden.size())
   // , biases_oh(output.size())
    {
    ...
    void propagate(void) {
        ...
        propagate_forward(hidden, weights_hi, td_input.get());
        vector_tanh(hidden);

        /* hidden to output layer */
        propagate_forward(output, weights_oh, hidden);
        vector_tanh(output);
    }
    ...
    void adapt(const TargetVector_t& targets, double learning_rate)
    {
        ...
        for (std::size_t i = 0; i < hidden.size(); ++i)
        {
            ...
            for (std::size_t j = 0; j < output.size(); ++j)
                error_i += weights_oh[j][i] * delta[j];

            /* adapt weights_hi */
            const double delta_i = error_i * tanh_(hidden[i]);
            for (std::size_t j = 0; j < output.size(); ++j)
                weights_hi[i][j] += learning_rate * delta_i * td_input.get()[j];
        }
        ...
        for (std::size_t i = 0; i < output.size(); ++i)
            for (std::size_t j = 0; j < hidden.size(); ++j)
                weights_oh[i][j] += learning_rate * delta[i] * hidden[j];
    }
    ...
    */
    ...
    matrix_t const& get_weights() const { return weights_oh; } /**TODO which weights?*/

    void randomize_weight_matrix(double random_weight_range)
    {
        randomize(weights_hi, random_weight_range);
        randomize(weights_oh, random_weight_range);
    }
};
